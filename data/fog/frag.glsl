...
{ 
  ...
  float d=sin(fp.x*0.3+1.0*4.0)+cos(fp.z*0.3+1.0*2.0);
  p.y=p.y+d;
  p.xz=sim2d(p.xz,2.0);
...
vec3 camera(
  ...
  in float vpd)
{
  vec2 vPos=-1.0+2.0*gl_FragCoord.xy/500.0;///iResolution.xy;
  vec3 vpn=normalize(vrp-prp);
  vec3 u=normalize(cross(vuv,vpn));
  vec3 v=cross(vpn,u);
  vec3 scrCoord=prp+vpn*vpd+vPos.x*u*500.0/500.0+vPos.y*v;
...
void main(void){
  ...
  //Camera animation
  vec3 vuv=vec3(0,1,0);
  vec3 vrp=vec3(4.0,0.0,0.0);
  float mx = 1.0*PI*2.0;
  float my=1.0*PI/2.01;
